...
const STATE_PREFIX = 'ops/inbound-state';
...
function extractInbound(reqBody = {}) {
  if (reqBody.event_type === 'message.received' && reqBody.message) {
    ...
    return {
      source: 'agentmail', eventType: reqBody.event_type, inboxId: m.inbox_id || '', threadId: m.thread_id || '', messageId: m.message_id || '',
      from: m.from_ || m.from || '', to: Array.isArray(m.to) ? m.to.join(', ') : (m.to || ''), cc: Array.isArray(m.cc) ? m.cc.join(', ') : (m.cc || ''),
      replyTo: m.reply_to || '', subject: m.subject || '(no subject)', text: m.text || '', html: m.html || '', preview: m.preview || '',
      timestamp: reqBody.created_at || new Date().toISOString(), raw: reqBody
    };
  }
  const payload = reqBody;
  if (payload.type && payload.type !== 'email.received') return { skip: true, source: 'resend', eventType: payload.type };
  const data = payload.data || payload;
  return {
    source: 'resend', eventType: payload.type || 'email.received', inboxId: data.to || '', threadId: '', messageId: data.message_id || '',
    from: data.from || data.sender || '', to: Array.isArray(data.to) ? data.to.join(', ') : (data.to || ''), cc: Array.isArray(data.cc) ? data.cc.join(', ') : (data.cc || ''),
    replyTo: data.reply_to || '', subject: data.subject || '(no subject)', text: data.text || data.body || '', html: data.html || '', preview: data.preview || '',
    timestamp: payload.created_at || new Date().toISOString(), raw: payload
  };
}
...
async function getThreadState(key) {
  ...
  try {
    ...
    if (String(e.message || '').startsWith('github_404')) return { path, sha: null, state: null };
    throw e;
  }
}

async function putThreadState(path, state, sha = null) {
  const body = {
    message: `Update inbound thread state for ${state.thread_key}`,
...
async function createPrivateIssue(inbound, senderKind, intentKind, key) {
  ...
  return githubRequest(`/repos/${PRIVATE_REPO}/issues`, {
    method: 'POST',
    body: {
...
async function addIssueComment(issueNumber, body) {
  return githubRequest(`/repos/${PRIVATE_REPO}/issues/${issueNumber}/comments`, { method: 'POST', body: { body } });
}
...
function fallbackReply(inbound, intentKind) {
  ...
  if (intentKind === 'booking') return { mode: 'fallback', action: 'reply', reason: 'fallback_booking', subject, text: `Hi ${firstName},\n\nYes — the fastest next step is to book an assessment call here: ${bookingLink}\n\nOn that call, I will figure out where AI can create the clearest leverage first and whether AI Native is the right next move.\n\n— Callias\nMilo`, html: `<p>Hi ${firstName},</p><p>Yes — the fastest next step is to book an assessment call here: <a href="${bookingLink}">${bookingLink}</a></p><p>On that call, I will figure out where AI can create the clearest leverage first and whether AI Native is the right next move.</p><p>— Callias<br>Milo</p>` };
  if (intentKind === 'pricing') return { mode: 'fallback', action: 'reply', reason: 'fallback_pricing', subject, text: `Hi ${firstName},\n\nMilo starts with a $500 assessment. AI Native Setup starts from $2,500 once the right first leverage point is clear.\n\nIf useful, book here and we can scope the next move: ${bookingLink}\n\n— Callias\nMilo`, html: `<p>Hi ${firstName},</p><p>Milo starts with a <strong>$500 assessment</strong>. AI Native Setup starts from <strong>$2,500</strong> once the right first leverage point is clear.</p><p>If useful, book here and we can scope the next move: <a href="${bookingLink}">${bookingLink}</a></p><p>— Callias<br>Milo</p>` };
  if (intentKind === 'support') return { mode: 'fallback', action: 'reply', reason: 'fallback_support', subject, text: `Hi ${firstName},\n\nGot this. If there is a deadline, blocker, or exact failure mode I should know about, reply here with that detail and I will use it.\n\n— Callias\nMilo`, html: `<p>Hi ${firstName},</p><p>Got this. If there is a deadline, blocker, or exact failure mode I should know about, reply here with that detail and I will use it.</p><p>— Callias<br>Milo</p>` };
  return { mode: 'fallback', action: 'reply', reason: 'fallback_general', subject, text: `Hi ${firstName},\n\nGot your note — I saw it. If the best next step is a call, you can book here: ${bookingLink}\n\nIf there is a deadline or context I should have before replying in more detail, send that over.\n\n— Callias\nMilo`, html: `<p>Hi ${firstName},</p><p>Got your note — I saw it. If the best next step is a call, you can book here: <a href="${bookingLink}">${bookingLink}</a></p><p>If there is a deadline or context I should have before replying in more detail, send that over.</p><p>— Callias<br>Milo</p>` };
}
...
async function sendReply(inbound, senderKind, reply) {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from: recipientInbox, to: [replyTarget], subject: reply.subject, text: reply.text, html: reply.html, reply_to: recipientInbox })
  });
  const data = await response.json();
  return { sent: response.ok, id: data.id || null, details: data };
}

async function autonomousReply(inbound, senderKind, intentKind, issue, comments, stateRef) {
  const state = stateRef.state || {};
  if (senderKind !== 'external') return { sent: false, suppressed: true, reason: 'not_external' };
  const processed = state.processed_message_ids || [];
  if (processed.includes(inbound.messageId)) return { sent: false, suppressed: true, reason: 'message_already_replied' };
  if (state.last_auto_reply_at && hoursSince(state.last_auto_reply_at) < AUTO_REPLY_SUPPRESSION_HOURS) {
    state.processed_message_ids = [...processed, inbound.messageId].slice(-50);
    await putThreadState(stateRef.path, state, stateRef.sha);
  ...
  if (reply.action === 'hold') {
    state.last_hold_at = new Date().toISOString();
    state.last_hold_reason = reply.reason || 'model_hold';
    state.processed_message_ids = [...processed, inbound.messageId].slice(-50);
    await putThreadState(stateRef.path, state, stateRef.sha);
    await addIssueComment(issue.number, `## auto_reply_hold\nheld_at: ${state.last_hold_at}\nreason: ${state.last_hold_reason}\ninbound_message_id: ${inbound.messageId || ''}`);
    return { sent: false, suppressed: false, held: true, reason: state.last_hold_reason, mode: reply.mode || 'gemini' };
  }
  const sendResult = await sendReply(inbound, senderKind, reply);
  if (sendResult.sent) {
    state.last_auto_reply_at = new Date().toISOString();
    state.last_auto_reply_mode = reply.mode || 'fallback';
    state.last_auto_reply_reason = reply.reason || '';
    state.processed_message_ids = [...processed, inbound.messageId].slice(-50);
    await putThreadState(stateRef.path, state, stateRef.sha);
    await addIssueComment(issue.number, ['## auto_reply', `sent_at: ${state.last_auto_reply_at}`, `mode: ${state.last_auto_reply_mode}`, `reason: ${state.last_auto_reply_reason}`, `inbound_message_id: ${inbound.messageId || ''}`, '', `subject: ${reply.subject}`, '', reply.text || ''].join('\n'));
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (inbound.skip) return res.status(200).json({ status: 'skipped', type: inbound.eventType, source: inbound.source });
    const senderKind = classifySender(inbound.from);
    const intentKind = classifyIntent(inbound);
    const key = threadKey(inbound);
    let stateRef = await getThreadState(key);
    let issue;
    if (stateRef.state && stateRef.state.issue_number) {
      issue = await githubRequest(`/repos/${PRIVATE_REPO}/issues/${stateRef.state.issue_number}`);
    } else {
      issue = await createPrivateIssue(inbound, senderKind, intentKind, key);
      const initialState = {
        thread_key: key,
      ...
      await putThreadState(stateRef.path, initialState, stateRef.sha);
      stateRef = await getThreadState(key);
    }
    await addIssueComment(issue.number, inboundCommentBody(inbound, senderKind, intentKind));
    const comments = await listIssueComments(issue.number);
    const replyResult = await autonomousReply(inbound, senderKind, intentKind, issue, comments, stateRef);
    console.log('INBOUND:', JSON.stringify({ source: inbound.source, sender_kind: senderKind, intent_kind: intentKind, from: inbound.from, to: inbound.to, subject: inbound.subject, inboxId: inbound.inboxId, threadId: inbound.threadId, private_issue: issue.number, reply_sent: replyResult.sent || false, reply_mode: replyResult.mode || null, reply_reason: replyResult.reason || null, suppressed: replyResult.suppressed || false, held: replyResult.held || false }));
    return res.status(200).json({ status: 'received', source: inbound.source, sender_kind: senderKind, intent_kind: intentKind, stored_privately: true, private_repo: PRIVATE_REPO, private_issue: issue.number, thread_key: key, autonomous_reply_sent: replyResult.sent || false, autonomous_reply_mode: replyResult.mode || null, reply_suppressed: replyResult.suppressed || false, reply_held: replyResult.held || false, reply_reason: replyResult.reason || null });
  } catch (err) {
